...
/**
 * Classe d'un paquet de jeu avec factory pour paquet de taille 32 et 52
...
public class Paquet extends AbstractModeleEcoutable {
    ...
    public void reset(int nb){
        String[] hauteur;
        ArrayList<String> couleur = new ArrayList<>(Arrays.asList("\u2665", "\u2663", "\u2660", "\u2666"));
        if(nb == 32){
            hauteur = hauteur32;
        }
        else{
    ...
    public int getSum(){
        ...
        for(Carte c : this.paquet){
            if(c.getVal().equals("J") || c.getVal().equals("Q") || c.getVal().equals("K")){
                sum += 10;
            }
            else if(c.getVal().equals("A") ) {
                as += 1;
            }
            else {
                sum += Integer.parseInt(c.getVal());
            }
        }
